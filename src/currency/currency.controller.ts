...
export class CurrencyController {
  ...
  async getRates(
    ...
    @Query("metadata") metadata: string | undefined,
    @Res() res: Response
  ) {
    try {
      const includeFields = metadata === "true" || metadata === "1";
      const wanted = currencies
        ? currencies.split(",").map((s) => s.trim().toUpperCase())
        : undefined;
      const payload = await this.currencyService.ratesRelativeTo(
        base.toUpperCase()
      );
      ...
      // if metadata is false, return a simple map { CODE: value }
      const result = includeFields
        ? {
            base: payload.base,
            time_last_update_unix: payload.time_last_update_unix,
            time_next_update_unix: payload.time_next_update_unix,
            data,
          }
        : {
            base: payload.base,
            ...
            data: Object.fromEntries(
              data.map((d: any) => [d.currency, d.value])
            ),
          };
